...
El marco dice **qué nivel de garantía exige cada trámite**. El banco de pruebas
dice **cómo se comprueba que el sistema implantado lo cumple**.

Sin lo primero, lo segundo no tiene contra qué medir. Sin lo segundo, lo primero
se queda en un documento.

---

## De dónde salen las pruebas

Las pruebas no se inventan: se derivan de lo que la norma exige para cada nivel.
El Reglamento de Ejecución (UE) 2015/1502 organiza sus especificaciones en
cuatro bloques. El banco reproduce esa estructura y añade una quinta área.
...
Esta clasificación es lo que distingue un banco de pruebas útil de una lista de
comprobaciones.

**Positiva.** El sistema acepta lo que debe aceptar. Es la más fácil de escribir
y la que todo el mundo hace.

**Negativa.** El sistema rechaza lo que debe rechazar. Es donde está la
protección real: un sistema que lo acepta todo supera cualquier prueba positiva.
...
encontrado. Las pruebas de traza son ese principio convertido en instrumento.

---

## Qué escala entre niveles

La norma no endurece todo a la vez. Endurece cosas concretas, y el banco de
pruebas se organiza sobre esas escaladas:
...
Cada uno de esos saltos genera pruebas distintas. Un sistema que aspire a nivel
alto tiene que superar las de alto y todas las anteriores.

---

## Cómo se sabe qué pruebas aplican a cada trámite

Una prueba aplica a un trámite cuando se cumplen dos condiciones:

1. El nivel exigido por el trámite alcanza o supera el nivel mínimo de la prueba.
2. Si la prueba requiere una condición concreta —por ejemplo, que el trámite
...
script `cobertura.py` cruza el marco con el catálogo y produce, para cada
trámite, la lista de pruebas que le corresponden. Y avisa en dos casos:

- Un trámite sin ninguna prueba aplicable en alguna área.
- Una prueba del catálogo que no aplica a ningún trámite, porque suele indicar
  una condición mal declarada.

Esa es la trazabilidad en los dos sentidos: del criterio a las pruebas y de las
pruebas al criterio. Ningún criterio se queda sin comprobar en silencio, y
ninguna prueba sobra sin que se note.
...
correcto, la prueba está mal escrita.

---

## Los documentos de prueba

Las pruebas de las áreas A1 y A2 necesitan documentos. Todos se fabrican para
...
El banco se ejecuta en dos momentos.
...
que superaba las pruebas el día de la entrega puede dejar de hacerlo tras una
...
**El banco comprueba comportamiento observable, no arquitectura interna.** Mide
...
**Superar el banco no acredita que un sistema sea seguro.** Acredita que cumple
los criterios del marco vigente en la fecha de ejecución. Son cosas distintas y
conviene no confundirlas.

**Las pruebas de resistencia frente a perfiles de atacante requieren
evaluación especializada.** Este banco comprueba que existen los controles y que
funcionan como se declara; no sustituye a una evaluación formal de resistencia
criptográfica.

**El catálogo no es exhaustivo.** Es un instrumento vivo: crece cuando aparece
un caso que ninguna prueba cubría. Cada incorporación queda versionada, de modo
que siempre se sabe contra qué versión del catálogo se ejecutó un acta.
